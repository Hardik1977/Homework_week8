 ...
 *
 */
public class DigitSumChallenge_Prog_4 {
    public static int sumDigits(int number) {

        if (number >= 10) {
            int digit, sum = 0;
            while (number > 0) {
                //look for a last digit input from user
                digit = number % 10;
                //adds last digit to the variable sum
                sum = sum + digit;
                //last number been removing
                number = number / 10;
            }
            return sum;

        } else {
            //retrun negative number and one number
            return -1;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Sum of number digits is :  " + result);  // Print output
